...
def get_flickr_photos(api_key, bbox, min_date, max_date, page=1):
    ...
    url = (f"https://www.flickr.com/services/rest/?method=flickr.photos.search"
           f"&api_key={api_key}&bbox={bbox}&extras=geo,date_taken,date_upload,owner_name,url_m"
...
def download_photo(photo, download_dir):
    """Download a single photo and save it to the specified directory"""
    try:
        # Check if URL for medium size photo is available
        if 'url_m' not in photo:
            print(f"  No download URL available for photo {photo['id']}")
            return False
        ...
        file_extension = os.path.splitext(photo_url.split('/')[-1])[1]
        if not file_extension:
            file_extension = '.jpg'  # Default to jpg if no extension found
        
        # Create filename with photo ID
        filename = f"{photo['id']}{file_extension}"
        filepath = os.path.join(download_dir, filename)
        
        # Skip if file already exists
        if os.path.exists(filepath):
            print(f"  Photo {photo['id']} already exists, skipping download")
            return True
        ...
        print(f"  Downloaded photo {photo['id']}")
        return True
...
def get_all_photos_in_shape(api_key, shapes, bbox, min_date, max_date, download_dir=None):
        ...
        for photo in data['photos']['photo']:
            try:
                ...
                if is_point_in_shape(lat, lon, shapes):
                    ...
                    # Download photo if download directory is specified
                    if download_dir:
...
def export_to_file(photos, filename):
    ...
    with open(filename, 'w', encoding='utf-8') as file:
        ...
        for photo in photos:
            ...
            file.write(f"{photo['id']};{photo['latitude']};{photo['longitude']};{photo['datetaken']};{photo['dateupload']};{photo['ownername']};{url}\n")
...
def main():
    ...
    parser.add_argument("--output", help="Output filename for metadata (default: flickr_photos_<timestamp>.txt)")
    parser.add_argument("--download_dir", help="Directory to download photos to (optional)")
    ...
    if args.download_dir:
        ...
        print(f"Photos will be downloaded to: {args.download_dir}")
    
    # Get photos
    print(f"Searching for photos between {min_date} and {max_date} within shapefile boundary...")
    photos = get_all_photos_in_shape(args.api_key, shapes, bbox_str, min_date, max_date, args.download_dir)
    ...
    print(f"\nSuccessfully exported {len(photos)} photos metadata to {output_file}")
    
    if args.download_dir:
        print(f"Photos downloaded to: {args.download_dir}")
